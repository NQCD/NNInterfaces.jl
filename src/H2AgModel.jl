...
struct H2AgModel{T} <: FrictionModels.AdiabaticFrictionModel
    ...
    function H2AgModel(h2indices=[1, 2])
        ...
        cd(splitdir(H2AgModel_jll.h2ag111_pes_path)[1]) do
            ccall((:pes_init_, H2AgModel.h2ag111_pes), Cvoid, ())
        end
...
function NonadiabaticModels.potential(model::H2AgModel, R::AbstractMatrix)
    ...
    ccall(((:pot0_, H2AgModel.h2ag111_pes)), Cvoid, (Ref{Int64}, Ref{Float64}, Ptr{Float64}),
...
function NonadiabaticModels.derivative!(model::H2AgModel, D::AbstractMatrix, R::AbstractMatrix)
    set_coordinates!(model, R)
    ccall((:dpeshon_, H2AgModel.h2ag111_pes), Cvoid, (Ref{Int64}, Ref{Float64}, Ptr{Float64}),
...
function FrictionModels.friction!(model::H2AgModel, F::AbstractMatrix, R::AbstractMatrix)
    ...
    cd(splitdir(H2AgModel_jll.h2ag111_friction_path)[1]) do
        ccall((:tensor_, H2AgModel.h2ag111_friction), Cvoid, (Ref{Float64}, Ptr{Float64}),
